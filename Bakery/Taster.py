...
class Taster:
    ...
    def compute_logli(self, precomputed=False):
        ...
        self.logli = pickle.load(open(self.folder + "/logli_buffer.p", "rb")).astype(np.float128)
        print(self.model_param_dict["dataname"]+f"max {np.max(self.logli)}, min {np.min(self.logli)}, mean {np.mean(self.logli)}, std {np.std(self.logli)}")
    ...
    def generate_image_forward_plot_matrix(self, dataname):
        ...
        best_repeats = np.argmax(self.logli_average_over_samples, axis=0)
        target_index = 6
